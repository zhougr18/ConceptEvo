...
class MAG(nn.Module):
    def __init__(self,  config, args):
        ...
        self.W_hv = nn.Linear(video_feat_dim + text_feat_dim, text_feat_dim) # Parameter (video_feat_dim + text_feat_dim) * text_feat_dim + text_feat_dim
    ...
    def forward(self, text_embedding, visual, acoustic):
        ...
        h_m = weight_v * self.W_v(visual) + weight_a * self.W_a(acoustic)  # [16,30,768] non-verbal displacement vector

        em_norm = text_embedding.norm(2, dim=-1) # [16,30]  L2 norm
        hm_norm = h_m.norm(2, dim=-1)  # [16,30]
        hm_norm_ones = torch.ones(hm_norm.shape, requires_grad=True).to(self.args.device)
        hm_norm = torch.where(hm_norm == 0, hm_norm_ones, hm_norm) # if hm_norm == 0, replace it to hm_norm_ones  Avoid zero elimination
        thresh_hold = (em_norm / (hm_norm + eps)) * self.beta_shift  # [16,30]

        ones = torch.ones(thresh_hold.shape, requires_grad=True).to(self.args.device)
        
        alpha = torch.min(thresh_hold, ones)  # [16,30]
        alpha = alpha.unsqueeze(dim=-1)  # [16,30,1]

        acoustic_vis_embedding = alpha * h_m   # [16,30,768] Multiplying corresponding elements
...
class MAG_BERT(nn.Module):
    def __init__(self, args):
        ...
        self.model = MAG_BertForSequenceClassification.from_pretrained(args.bert_base_uncased_path, args = args)
    
    def forward(self, text_feats, video_feats, audio_feats):
        
        video_feats, audio_feats = video_feats.float(), audio_feats.float()

        outputs = self.model(text = text_feats, visual = video_feats,acoustic = audio_feats)
