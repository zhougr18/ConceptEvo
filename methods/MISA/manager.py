...
class MISA:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Iteration")):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    args.recon_weight * recon_loss  # 0.7 0.7 0.6
    ...
    def _get_domain_loss(self):
        ...
        domain_pred_t = self.model.model.domain_label_t  # [16,3]
        domain_pred_v = self.model.model.domain_label_v  # [16,3]
        domain_pred_a = self.model.model.domain_label_a  # [16,3]

        # True domain labels
        domain_true_t = torch.LongTensor([0]*domain_pred_t.size(0)).to(self.device)  # [16]
        domain_true_v = torch.LongTensor([1]*domain_pred_v.size(0)).to(self.device)  # [16]
        domain_true_a = torch.LongTensor([2]*domain_pred_a.size(0)).to(self.device)  # [16]

        # Stack up predictions and true labels
        domain_pred = torch.cat((domain_pred_t, domain_pred_v, domain_pred_a), dim=0)  # [48,3]
        domain_true = torch.cat((domain_true_t, domain_true_v, domain_true_a), dim=0)  # [48,3]
    ...
    def _get_diff_loss(self):
        ...
        shared_t = self.model.model.utt_shared_t  # [16,256]
        shared_v = self.model.model.utt_shared_v  # [16,256]
        shared_a = self.model.model.utt_shared_a  # [16,256]
        private_t = self.model.model.utt_private_t  # [16,256]
        private_v = self.model.model.utt_private_v  # [16,256]
        private_a = self.model.model.utt_private_a  # [16,256]
